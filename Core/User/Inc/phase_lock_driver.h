...
#define PHASE_LOCK_KP_A 1.0f /* 比例系数 (Hz/V), 1V误差→1Hz比例修正 */
#define PHASE_LOCK_KI_A 0.0005f /* 积分系数 (Hz/V, 已预乘 dt=0.0002s, Ki_raw=2.5) */
#define PHASE_LOCK_KD_A 0.0f /* 微分系数 — 先关闭 */

/* 锁相放大器 B (ADS8688 CH1 → AD9833 hspi4) */
#define PHASE_LOCK_KP_B 1.0f /* 比例系数 (Hz/V) */
#define PHASE_LOCK_KI_B 0.0005f /* 积分系数 (Hz/V, 已预乘 dt=0.0002s, Ki_raw=2.5) */
#define PHASE_LOCK_KD_B 0.0f /* 微分系数 — 先关闭 */

/* 通用限幅 */
#define PHASE_LOCK_INTEGRAL_LIMIT 100.0f /* 积分项限幅 (Hz) */
#define PHASE_LOCK_MAX_FREQ_STEP 10.0f /* 单次最大频率步进 (Hz), 防振荡 */

/* 锁相目标电压 (V) — 仅作为初始化回退值，实际目标由首次采样动态捕获 */
#define PHASE_LOCK_TARGET_VOLTAGE 0.0f

/* 目标电压捕获延迟 (TIM7 周期数) — 等待锁相放大器 LPF 稳定后再捕获 */
#define PHASE_LOCK_SETTLE_TICKS 250 /* 50ms @ 5kHz TIM7 */

/* 频率下限保护 — 防止频率漂移到 0 Hz */
#define PHASE_LOCK_MIN_FREQ_HZ 5000.0f /* 最低允许输出频率 (Hz)，对应最小步进 5kHz */

/* 电压死区 (V) — 锁相放大器输出在此范围内时不进行 PID 修正，防止振荡 */
#define PHASE_LOCK_VOLTAGE_DEADBAND 0.005f
